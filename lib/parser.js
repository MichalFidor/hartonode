...
var Parser = /** @class */ (function () {
    ...
    Parser.prototype.harToNode = function (har) {
        var prettyJson = this.harToPrettyJson(har);
        return parser.toNode(prettyJson);
    };
    Parser.prototype.harToPrettyJson = function (har) {
    ...
    Parser.prototype.toNode = function (request) {
    ...
    Parser.prototype.getHeaders = function (request) {
        var nodeCode = '';
    ...
    Parser.prototype.matchUrl = function (url, group) {
        var PATTERN_WITH_HTTP = /(https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6})(\/.+)/gm;
        var PATTERN_WITHOUT_HTTP = /([-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6})(\/.+)/gm;
        var urlMatched;
    ...
    Parser.prototype.fromLog = function (log) {
        if (!log || !Array.isArray(log.entries)) {
            return;
        }
        return this.fromEntries(log.entries);
    };
    Parser.prototype.fromEntries = function (entries) {
        return entries.map(this.fromEntry);
    };
    Parser.prototype.fromEntry = function (entry) {
        ...
        if (entry.request.cookies.length) {
            entry.request.cookies.map(function (cookie) {
                return command.cookies[encodeURIComponent(cookie.name)] = encodeURIComponent(cookie.value);
            });
        }
        else {
            delete command.cookies;
        }
        entry.request.headers.map(function (header) {
            return command.headers[header.name] = header.value;
        });
